function J = phase_diversity_n_wavelengths(zernike_coeff,data,wavelengths,S,pupil)
...
for ii=1:1:length(wavelengths);
    ...
    zernike_coeff_tmp = zernike_coeff * (wavelengths(1) / wavelengths(ii));
    ...
    Image_plane_error(ii) = sum(sum( ( data(:,:,ii) - H_PSF ).^2));
